...
const StreakDisplay = ({ streak, lastActivityDate, streakBlocks = 2, lastBlockReset, className }: StreakDisplayProps) => {
    ...
    <div className={cn("p-4 rounded-xl bg-card text-card-foreground", className)}>
      <div className="flex items-center mb-3">
        <div className={cn(
          ...
          {active && (
            <div className="absolute -inset-1 rounded-full bg-levelup-accent/20 animate-[ping_2s_ease-in-out_infinite]" />
          )}
        </div>
        ...
        <div className="flex">
          {[...Array(2)].map((_, i) => (
            <Heart
              key={i}
              fill={i < streakBlocks ? "red" : "none"}
              className={cn(
                "w-6 h-6 mx-0.5", 
                i < streakBlocks ? "text-red-500" : "text-muted"
              )}
            />
          ))}
        </div>
