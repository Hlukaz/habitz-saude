  ...
  return (
    <div className={cn("p-4 rounded-xl bg-card text-card-foreground", className)}>
        ...
        <div className="flex">
          {[...Array(2)].map((_, i) => (
            <Heart 
              ...
              className={cn("w-6 h-6 mx-0.5", i < streakBlocks ? "text-red-500" : "text-muted")} 
            />
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-2">
        {[1, 2, 3].map(day => (
          <div key={day} className="bg-background rounded-md p-2 text-center">
            <span className="text-xs">Dia {day}</span>
          </div>
        ))}
      </div>
